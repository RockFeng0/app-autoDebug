...
#         print grid_tree        
    ...
    def confirm(self):
        ...
        if self.__rpcclt:
            self.__rpcclt.remotequeue("exec", "runcode", ('a.testSet["%s"].update(%r)' %(sc.CASE["id"],sc.CASE["value"])), {})
...
#             api_file_path = r"D:\auto\python\app-autoDebug\haloudebug\UIWeb.py"
    ...
    def __sel_file(self,*args, **kwargs):
        ...
        if args[0] == "load_cases":
            ...
            if not excel_name:
                excel_path = FileDilog.Askopenfilename(filetypes = [("Excel","*.xlsx"),("Excel2003","*.xls")],initialdir = None)
                if not excel_path:
                    return
    ...
    def __runner(self,*args):
        ...
        if args[0] == "all":
            ...
            tmp = []
            for i in excels:
                if not os.path.isfile(os.path.join(p_env.DATA_PATH, i)):
                    tmp.append(i)                                        
                    continue
            if tmp:
