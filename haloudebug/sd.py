...
********************************************************************
    ...
    RCS:      haloudebug.sd,v 1.0 2016年8月30日
    FROM:   2016年8月30日
********************************************************************
...
from pyrunner.ext.idleshell.diyrpc import MyRpcClient
...
class ElemFeatureXML(XmlDpc):
    ...
    def __setWebFeature(self):
        self.setFeature(str_feature = self.__feature["str"],
                        int_feature = self.__feature["int"],
                        attr_feature = self.__feature["attr"],
                        class_feature = self.__feature["class"],
...
class SdRunner:
    def __init__(self,section, proj_name = "appDebugProject", debug = False):        
        self.proj_path = self.get_exe_path(proj_name,debug = debug)
        tracer.set_proj_path(proj_name, self.proj_path, sysencoding = False)
        self.itemrst = None
    ...
    def start_run(self, mname, caseid = None):
        if isinstance(mname, types.ModuleType):
            ...
            module_names = {"web":"UIWeb","pc":"UIPc","pad":"UIPad"}
            if not mname in module_names:
                raise Exception("Need specify web/pc/pad. not '%s'" %mname)
        ...
        for fs,caseid in status:
            ...
            judgement(test_result[0], test_result[1], caseid)            
            if not caseid:
                self.itemrst = (caseid,p_env.CASE_PASS)

    def get_exe_path(self,proj_name,debug = True):
        if debug:
            #调试路径
            lib_path = os.path.dirname(os.path.abspath(sys.path[0]))            
        else:
            lib_path = os.path.abspath(sys.path[0])
            
        print os.path.join(lib_path, "UIWeb.py")
        print os.path.join(lib_path, "UIPc.py")
        print os.path.join(lib_path, "UIPad.py")
